...
public class SpriteUtils {
    ...
    private HashMap<CountryFlags, BufferedImage> flagMap;
    ...
    private void loadExtras() throws IOException {
        SpriteParser parser = new SpriteParser("resources/sprites/extraunits.png", "resources/sprites/extraunits.sprites");
        Tuple<String, BufferedImage> spritePair;
        while ((spritePair = parser.next()) != null) {
            unitMap.put(UnitType.valueOf(spritePair.x), spritePair.y);
        }
    ...
    {
        this.flagMap = new HashMap<CountryFlags, BufferedImage>();
        SpriteParser parser = new SpriteParser("resources/sprites/flags.png", "resources/sprites/flags.sprites");
        Tuple<String, BufferedImage> spritePair;
        while ((spritePair = parser.next())!=null)
        {
            flagMap.put(CountryFlags.valueOf(spritePair.x), spritePair.y);
        }
    }
    ...
    public BufferedImage getTerrainSprite(MapLocation coords) {
        try {
            ...
            if(tile.getBonusType() != TerrainBonusType.NONE && world.getCity(coords) == null)
            {
    ...
    public BufferedImage getUnitSprite(MapLocation mapLocation) {
        ...
        if(engine.getCurrentlySelectedUnitTile() != null && engine.getCurrentlySelectedUnitTile().compare(mapLocation));
        {
            Unit selUnit = engine.getCurrentlySelectedUnit();
            for(Unit u : units)
            {
                if(u.compare(selUnit))
                    tempUnit = selUnit;
            }
        }
        return getUnitSprite(tempUnit);
    }


    public BufferedImage getUnitSprite(Unit unit)
    {
        BufferedImage baseImage = unitMap.get(unit.getUnitType());

        BufferedImage clonedBaseImage = cloneSprite(baseImage);
        Graphics2D paintRegion = clonedBaseImage.createGraphics();

        paintRegion.drawImage(unitDecorationMap.get(unit.getHealthRatio()), 0, 0, TILEWIDTH, TILEHEIGHT, null);
        
        if (unit.getNumUnits() > 0) {
            paintRegion.drawImage(unitDecorationMap.get("TRANSPORT"), 0, 0, TILEWIDTH, TILEHEIGHT, null);
        }
        
        paintRegion.drawImage(flagMap.get(unit.getOwner().getCountry()), 0, 0, TILEWIDTH, TILEHEIGHT, null);

        if(unit.getState() == Unit.State.MOVING || unit.getState() == Unit.State.MOVE_BLOCKED) {
    ...
    public BufferedImage getCitySprite(MapLocation mapLocation) {
        ...
        if (city.getLevel() < 5) {
            sprite = cityMap.get(city.getStyle().toString() + "_" + 1);
        } else if (city.getLevel() < 10) {
            sprite = cityMap.get(city.getStyle().toString() + "_" + 2);
        } else {
            sprite = cityMap.get(city.getStyle().toString() + "_" + 3);
        }

        BufferedImage result = cloneSprite(sprite);
        Graphics2D paintRegion = result.createGraphics();
        paintRegion.drawImage(flagMap.get(city.getOwner().getCountry()), 0, 0, TILEWIDTH, TILEHEIGHT, null);
        return result;
    }
    ...
    public Color getTerrainColor(int x, int y)
    {
        ...
        if((crets.isEmpty() &&loc == null)||engine==null)
