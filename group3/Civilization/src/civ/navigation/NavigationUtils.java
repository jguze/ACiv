...
public class NavigationUtils {
    ...
    private boolean isAllowedToMove(Unit unit, MapLocation mapLocation) {
        if(unit.getMovementType() == UnitTravelType.SEA) {
            return world.getTile(mapLocation).getTerrainType() == TerrainType.OCEAN || cityHasWater(mapLocation);
        } else if(unit.getMovementType() == UnitTravelType.GROUND) {
            if(world.getUnitList(mapLocation).isEmpty() == false && world.getUnitList(mapLocation).get(0).isTransport()==true)return true;
    ...
    private boolean cityHasWater(MapLocation mapLocation) {
        City city = world.getCity(mapLocation);
        if (city == null) {
            return false;
        }
        
        ArrayList<Integer> aroundX = getAroundX(mapLocation.x);
        ArrayList<Integer> aroundY = getAroundY(mapLocation.y);
        
        boolean result = world.getTile(new MapLocation(aroundX.get(0), mapLocation.y)).getTerrainType() == TerrainType.OCEAN ? true : 
                world.getTile(new MapLocation(aroundX.get(2), mapLocation.y)).getTerrainType() == TerrainType.OCEAN ? true :
                world.getTile(new MapLocation(mapLocation.x, aroundY.get(0))).getTerrainType() == TerrainType.OCEAN ? true : 
                world.getTile(new MapLocation(mapLocation.x, aroundY.get(2))).getTerrainType() == TerrainType.OCEAN ? true : false;
